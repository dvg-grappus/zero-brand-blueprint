...
export const SecondaryInsightPool: React.FC<SecondaryInsightPoolProps> = ({ isOpen, onClose }) => {
  const { secondaryInsights, starInsight, deleteInsight, mergeInsights } = useCompetition();
  ...
  const handleMergeSelected = () => {
    ...
    const mergedText = `Combined insight: ${toMerge.map(i => i?.text).join(" + ")}`;
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-lg font-semibold text-foreground flex items-center">
                  ...
                  Secondary Pool ({secondaryInsights.length})
                </h2>
                <button 
                  onClick={onClose}
                  ...
                  >
                    <div className="flex items-start mb-2">
                      ...
                      <div 
                        className={`w-3 h-3 rounded-full mt-1 ml-2 flex-shrink-0 ${
                          insight.category === "takeaway" 
                            ? "bg-blue-500" 
                            : insight.category === "trend" 
                            ? "bg-green-500" 
                            : insight.category === "competitor" 
                            ? "bg-purple-500"
                            : "bg-amber-500"
                        }`}
                      />
                    ...
                    <div className="flex justify-between items-center mt-2 text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => starInsight(insight.id, !insight.starred)}
                          className={`${
                ...
                {secondaryInsights.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm mt-2">
                      Insights will be collected as you progress through the competition workflow.
                    </p>
                  </div>
                )}
              </div>
